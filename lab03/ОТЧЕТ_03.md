...
3.	Включил в `psql` параметр `ON_ERROR_ROLLBACK`. создала ситуацию с ошибкой в транзакции и убедился, что этот режим использует точки сохранения (`SAVEPOINT`), позволяя продолжить работу транзакции после ошибки.
