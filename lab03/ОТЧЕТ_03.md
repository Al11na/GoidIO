...
## Теоретическая часть
**MVCC (Multiversion Concurrency Control)** — механизм, позволяющий нескольким транзакциям работать с одними и теми же данными одновременно, минимизируя блокировки. Каждая транзакция видит согласованный «снимок» данных на момент своего началаа. 
...
4. Снимок данных (Snapshot): Набор идентификаторов транзакций, активных на момент началаа текущей транзакции. Определяет, какие версии строк видимы текущей транзакции.

## Практическая часть

### Часть 1. Уровни изоляции и аномалии
**Выполненные задачи:**   
1. Создала таблицу `iso_test (id INT, data TEXT)` и вставила одну строку. В сеансе 1 начала транзакцию с уровнем `READ COMMITTED` и выполнила `SELECT iso_test;. * FROM iso_test;`. В сеансе 2 удалил строку и зафиксировал изменения (`DELETE ...; COMMIT;`). В сеансе 1 выполнила тот же `SELECT` повторно. Вывело 0 строк. Завершила транзакцию в сеансе 1.

2. Повторила предыдущий эксперимент, но в сеансе 1 начала транзакцию с `BEGIN ISOLATION LEVEL REPEATABLE READ;`. `READ COMMITTED` берёт новый снимок на каждый запрос. `REPEATABLE READ` фиксирует снимок данных и держит его неизменным на всю транзакцию.

3. В сеансе 1 начала транзакцию и создала новую таблицу `new_table`, вставила в нее строку. Не фиксировал. В сеансе 2 выполнила `SELECT * FROM new_table;`. Выдало ошибку о несуществовании таблицы. Зафиксировала транзакцию в сеансе 1. Повторила запрос в сеансе 2, таблица появилась. Повторила процесс, но вместо фиксации откатил транзакцию в сеансе 1. После `ROLLBACK;` таблица также нету в сеансе 2.

4. В сеансе 1 начала транзакцию и выполнила `SELECT * FROM iso_test;`. Попыталась в сеансе 2 выполнить `DROP TABLE iso_test;`. Удалить таблицу не получится, пока не завершим транзакцию в первом сеансе.
...
**Выполненные задачи:**   
1.	Создала пустую таблицу `phantom_test (id INT)`. Продемонстрировала на уровне `Read Committed`, что аномалия "фантомное чтение" не предотвращается (вставка новых строк в другом сеансе становится видимой).

2.	В сеансе 1 начала транзакцию с уровнем `Repeatable Read` (пока без запросов). В сеансе 2 удалила все строки из `phantom_test` и зафиксировала. В сеансе 1 выполнила `SELECT * FROM phantom_test;`. Удалённые стркои вывелись. Выполнила в сеансе 1 запрос `SELECT * FROM pg_database;` (не касаясь `phantom_test`). При последующем запросе также вывело удалённые строки.

3.	Убедилась, что `DROP TABLE` является транзакционной операцией (можно откатить).
...
**Выполненные задачи:**   
1.	Создала таблицу `version_test (id INT)`. вставила одну строку. Дважды обновила эту строку (`UPDATE ...`), а затем удалил её (`DELETE`). Используя расширение `pageinspect` (`heap_page_items`), сейчас версий строк 3. Все версии погашены, так как t_xmax не равен 0. По t_ctid видно, что первая версия строки ссылается на вторую версия, так как вторая удалила первую версию. Вторая версия переадресовывает на третью версию. Так как это была последняя версия строки она ссылается на саму себя.

2.  Определила, в какой странице (блоке) находится строка в `pg_class`, описывающая саму таблицу `pg_class`. Используя `pageinspect`, Актуальных версий строк 2, так как только у двух строк t_xmax = 0.

3.	Включила в `psql` параметр `ON_ERROR_ROLLBACK`. Создала ситуацию с ошибкой в транзакции и убедилась, что этот режим использует точки сохранения (`SAVEPOINT`), позволяя продолжить работу транзакции после ошибки.
...
**Выполненные задачи:**   
1.	Воспроизвела ситуацию, при которой одна транзакция (A) видит строку, а другая (B), начавшаяся позже, — уже нет (строка удалена и зафиксирована после началаа A, но до началаа B). Использовала функции `pg_current_snapshot()` и `pg_snapshot_xip(pg_current_snapshot())` для анализа снимков обеих транзакций. Изучила значения `xmin` и `xmax` удаленной строки. Снимок транзакции A — 879:879:, t_xmin = 878, вставка видна. t_xmax = 880 > xmax = 879. Снимок транзакции B — 881:881:, t_xmax = 880 < xmin = 881 и удаляющая транзакция к этому моменту зафиксирована - строка невидима.

2.	Создала функцию `STABLE`, возвращающую данные из таблицы. Исследовала, какой снимок данных используется для запроса внутри этой функции при разных уровнях изоляции (`Read Committed` и `Repeatable Read`). Повторила для функции `VOLATILE`. На `Read Committed` обе функции вернули новое значение, так как запрос внутри функции использует снимок текущего оператора, а снимок на `Read Committed` меняется от запроса к запросу. На `Repeatable Read` обе функции неизменно возвращают одно и то же — они читают снимок транзакции, зафиксированный первым запросом. `STABLE` и `VOLATILE` не влияет на видимость данных (снимок), только на то, может ли планировщик переиспользовать результат внутри одного оператора.

3.	В транзакции 1 (уровень `Repeatable Read`) экспортировала снимок данных с помощью `pg_export_snapshot()`. В транзакции 2 изменила данные и зафиксировала. В транзакции 3 импортировала снимок из транзакции 1 (`SET TRANSACTION SNAPSHOT '...';`). Убедилась, что в транзакции 3 видны данные в состоянии на момент экспорта снимка, до изменений из транзакции 2.
...
6. **Режим автоматических точек сохранения ON_ERROR_ROLLBACK.**  
   Включение параметра `\set ON_ERROR_ROLLBACK on` в psql позволило продолжить работу транзакции после возникновения ошибки (деление на ноль). PostgreSQL автоматически создала `SAVEPOINT` перед каждой командой, откатив только проблемную операцию без прерывания всей транзакции. Это упрощает интерактивную отладку сложных скриптов.
...
7. **Анализ снимков транзакций и видимость версий строк.**  
   ...
   - Транзакция A (более ранняя) видит строку, так как удаляющая транзакция началаась после создания её снимка (`xmax` > `xmin` снимка A).
   - Транзакция B (более поздняя) не видит строку, поскольку удаляющая транзакция Завершилаась до создания снимка B.
...
## Выводы
При выполнении данной лабораторной работы были изучены принципы многоверсионного управления конкурентным доступом (MVCC) в PostgreSQL, были получены практические навыки наблюдения за работой MVCC, анализа версий строк, снимков данных и уровней изоляции транзакций. Освоены использование расширений и системных представлений для исследования внутренней структуры данных.
